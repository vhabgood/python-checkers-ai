# endgame_generator_2K1Mv3K_V4.py
import pickle
...
def get_moves(position):
    # Position: (rk1, rk2, rm, wk1, wk2, wk3, turn)
    rk_sqs, rm_sq, wk_sqs, turn = set(position[:2]), position[2], set(position[3:6]), position[6]
...
def generate_2K1Mv3K_positions():
    positions = set()
    print("Generating 2K1M vs 3K positions...")
...
if __name__ == '__main__':
    ...
    for pos in all_positions:
        if not get_moves(pos):
            # White (3K) wins if it can capture a piece.
            endgame_db[pos] = -1 if pos[6] == 'w' else 1
    ...
    while newly_solved:
        ...
        for pos in all_positions:
            ...
            turn = pos[6]
            # Red's turn (weaker side), must find a loss
            if turn == 'r':
                target = ply - 1 # Look for moves into a known white win
                if all(endgame_db.get(move) == target for move in moves):
                    endgame_db[pos] = -ply; newly_solved.append(pos)
            # White's turn (stronger side), can find a win
            else:
                target = -(ply - 1) # Look for a move into a known red loss
