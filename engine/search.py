# engine/search.py
from .constants import Checkers, EMPTY, RED, WHITE
from .evaluation import evaluate_board_static

def _get_mvv_lva_score(board, move):
    # This helper function is now local to the search module
    start, end = move
...
def _record_killer_move(move, depth, killer_moves):
    # This helper function is also local to the search module
...
def _static_quiescence_search(game, board, turn, alpha, beta, maximizing_player, eval_counter):
    ...
    if entry:
        # For quiescence, we only need a score, not a full path
        return entry['score'], []
    ...
    if maximizing_player:
        ...
        for start, end in capture_moves:
            temp_game = Checkers([row[:] for row in board], turn, load_resources=False)
            temp_game.hash = game.hash
            further_jumps = temp_game.perform_move_for_search(start, end)
            value, _ = _static_quiescence_search(temp_game, temp_game.board, temp_game.turn, alpha, beta, bool(further_jumps), eval_counter)
        ...
        for start, end in capture_moves:
            ...
            temp_game.hash = game.hash
            further_jumps = temp_game.perform_move_for_search(start, end)
            value, _ = _static_quiescence_search(temp_game, temp_game.board, temp_game.turn, alpha, beta, not further_jumps, eval_counter)
...
def static_minimax(game, board, turn, depth, alpha, beta, maximizing_player, eval_counter, progress_callback, killer_moves, path):
    ...
    if maximizing_player:
        ...
        for i, (start, end) in enumerate(ordered_moves):
            ...
            if depth <= 2 and not is_capture and not any(abs(s[0]-e[0])==2 for s,e in temp_game.get_all_possible_moves(temp_game.turn)):
            ...
            eval_score, sub_path = static_minimax(temp_game, temp_game.board, temp_game.turn, depth - 1, alpha, beta, not temp_game.turn == turn, eval_counter, progress_callback, killer_moves, path + [(start, end)])
        ...
        for i, (start, end) in enumerate(ordered_moves):
            ...
            if progress_callback: progress_callback(None, None, path + [(start, end)])
            eval_score, sub_path = static_minimax(temp_game, temp_game.board, temp_game.turn, depth - 1, alpha, beta, temp_game.turn == turn, eval_counter, progress_callback, killer_moves, path + [(start, end)])
